...
public class AuthController {
	...
	public ResponseEntity<?> registerUser(@Valid @RequestBody SignupRequest signUpRequest) {
		...
		User user = new User(signUpRequest.getUsername(), 
							 ...
							 encoder.encode(signUpRequest.getPassword()), signUpRequest.getHeight(), signUpRequest.getWeight(), signUpRequest.getSex());
		...
		if (strRoles == null) {
			Role userRole = roleRepository.findByName(ERole.ROLE_USER)
					.orElseThrow(() -> new RuntimeException("Error: Role no se encontró."));
			roles.add(userRole);
		} else {
		...
		//primera asociacion
		if(userService.checkIfRoleUser(user)) {
			userService.firstAssociation(user);
			userRepository.save(user);
		}
